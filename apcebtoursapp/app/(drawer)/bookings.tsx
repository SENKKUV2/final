...
export default function BookingsScreen() {
  ...
  const [modal, setModal] = useState<{ visible: boolean; type: 'edit' | 'view'; booking: Booking | null }>({
    visible: false,
    type: 'view',
    booking: null,
  });
  ...
  const fetchBookings = async (id?: string) => {
    ...
    const query = supabase
      .from('bookings')
      .select('*, profiles ( full_name, first_name, last_name ), tours ( title )')
    ...
    if (error) {
      ...
      setMessageModal({
        visible: true,
        title: 'Error',
        message: `Failed to fetch ${id ? 'booking details' : 'bookings'}.`,
        type: 'error',
      });
      setLoading(false);
      return id ? null : [];
  ...
  const closeModal = () => setModal({ visible: false, type: 'view', booking: null });

  const closeMessageModal = () => setMessageModal({ visible: false, title: '', message: '', type: 'success' });
      ...
      setBookings(prev =>
        prev.map(b =>
          b.id === modal.booking?.id
            ? { ...b, number_of_people: parseInt(formData.numberOfPeople), special_requests: formData.specialRequests, contact_phone: formData.contactPhone }
            : b
        )
      );
      setMessageModal({
  ...
  const filteredBookings = bookings.filter(b => {
    const matchesSearch =
      ...
      getDisplayName(b).toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const getStatusColor = (status: string) => ({
    ...
    completed: '#2196F3',
    cancelled: '#F44336',
    'cancel-requested': '#FF5722',
  }[status] || '#666');

  const getStatusIcon = (status: string): MaterialIconName => statusIconMap[status] || 'help';
          ...
          <View style={styles.modalContent}>
            ...
            {isEdit ? (
              ...
              <View style={styles.actionButtons}>
                ...
                <TouchableOpacity style={[styles.actionButton, styles.rejectButton]} onPress={() => handleStatusChange(modal.booking!.id, 'confirmed')}>
  ...
  const renderMessageModal = () => {
    if (!messageModal.visible) return null;
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBody}>
              <Text style={[styles.messageText, { color: messageModal.type === 'success' ? '#4CAF50' : '#F44336' }]}>
                {messageModal.message}
              </Text>
            </View>
            <TouchableOpacity style={styles.saveButton} onPress={closeMessageModal}>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filterContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScrollView}>
          {['All', 'Pending', 'Confirmed', 'Completed', 'Cancel-Requested', 'Cancelled'].map(filter => (
            <TouchableOpacity
              key={filter}
            ...
            >
              ...
              {filter !== 'All' && (
                <View style={[styles.filterBadge, { backgroundColor: filter === activeFilter ? '#fff' : getStatusColor(filter.toLowerCase()) }]}>
                  <Text style={[styles.filterBadgeText, { color: filter === activeFilter ? getStatusColor(filter.toLowerCase()) : '#fff' }]}>
                    {filter === 'Cancel-Requested' ? bookings.filter(b => b.status === 'cancel-requested').length : bookings.filter(b => b.status === filter.toLowerCase()).length}
                  </Text>
                </View>
              )}
            </TouchableOpacity>
                ...
                <View style={styles.actionButtons}>
                  ...
                  {booking.status === 'cancel-requested' && (
                    <>
                      ...
                      <TouchableOpacity style={[styles.actionButton, styles.rejectButton]} onPress={() => handleStatusChange(booking.id, 'confirmed')}>
                        <MaterialIcons name="close" size={16} color="#fff" />
...
const styles = StyleSheet.create({
  ...
  activeFilterTab: { backgroundColor: '#f57c00', borderColor: '#f57c00' },
  ...
  bookingAmount: { fontSize: 16, fontWeight: 'bold', color: '#f57c00' },
  ...
  completeButton: { backgroundColor: '#1976D2' },
  rejectButton: { backgroundColor: '#F44336' },
  viewButton: { backgroundColor: '#666' },
  actionButtonText: { color: '#fff', fontSize: 13, fontWeight: '600', marginLeft: 4 },
  ...
  modalContent: { backgroundColor: '#fff', borderRadius: 12, width: '100%', maxWidth: 400, padding: 16 },
  ...
  saveButton: { backgroundColor: '#f57c00', paddingVertical: 12, borderRadius: 8, alignItems: 'center' },
